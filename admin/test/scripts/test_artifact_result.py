...
__artifacts_v2__ = {
    'streaming_module_that_raises': {
        ...
        'notes': '', 'paths': ('*/nothing',), 'output_types': ['lava'],
    },
...
class TestARaisingModuleLeavesNothingBehind(ArtifactResultTestCase):
    ...
    def test_streaming_module_that_raises_leaves_no_table_and_no_manifest_entry(self):
        with self.assertRaises(RuntimeError):
            streaming_module_that_raises([], self.tmpdir, None, True)  # pylint: disable=too-many-function-args
    ...
    def test_list_module_that_raises_leaves_the_same_nothing(self):
        with self.assertRaises(RuntimeError):
            list_module_that_raises([], self.tmpdir, None, True)  # pylint: disable=too-many-function-args
        self.assertEqual(self._artifact_tables(), [])
        self.assertNotIn('Probe Category', lavafuncs.lava_data['artifacts'])
...
class TestReplayKeepsPythonText(ArtifactResultTestCase):
    ...
    def test_list_path_storage_is_unchanged(self):
        ...
        # SQLite's rendering of a bound float and boolean into TEXT columns, and a float
        # epoch for an ISO string: what this core's list path stored before this change.
        self.assertEqual(stored, ('35.6595662310191', '1', 1721575007.357843))
...
class TestRowsIterableEmptyCase(ArtifactResultTestCase):
    ...
    def test_an_empty_iterable_module_leaves_no_table_and_no_manifest_entry(self):
        rows_iterable_module_empty([], self.tmpdir, None, True)  # pylint: disable=too-many-function-args
        self.assertEqual(self._artifact_tables(), [])
        self.assertNotIn('Probe Category', lavafuncs.lava_data['artifacts'])

    def test_a_non_empty_iterable_module_writes_every_row(self):
        rows_iterable_module_rows([], self.tmpdir, None, True)  # pylint: disable=too-many-function-args
