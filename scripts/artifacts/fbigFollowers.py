__artifacts_v2__ = {
    "fbigFollowers": {
        ...
        "description": "Followers parsed from a Facebook/Instagram law enforcement return (records.html / preservation).",
...
def fbigFollowers(context):
    ...
    for file_found in context.get_files_found():
        ...
        sections = soup.find_all('div', {'id': 'property-followers'})
        if not sections:
            continue
        chunk = []
        for index, div in enumerate(sections[0].find_all('div', class_='div_table inner')):
            ...
            if len(chunk) == 2:
                data_list.append((_fbig_ts(chunk[1]), chunk[0]))
                chunk = []
