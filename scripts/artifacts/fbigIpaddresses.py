__artifacts_v2__ = {
    "fbigIpaddresses": {
        ...
        "description": "Account IP address activity parsed from a Facebook/Instagram law enforcement return (records.html / preservation).",
...
def fbigIpaddresses(context):
    ...
    for file_found in context.get_files_found():
        ...
        sections = soup.find_all('div', {'id': 'property-ip_addresses'})
        if not sections:
            continue
        chunk = []
        for index, div in enumerate(sections[0].find_all('div', class_='div_table inner')):
            ...
            if len(chunk) == 3:
                data_list.append((_fbig_ts(chunk[1]), chunk[0], chunk[2]))
                chunk = []

    data_headers = (('Timestamp', 'datetime'), 'IP', 'Action')
