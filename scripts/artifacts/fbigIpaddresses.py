...
def get_fbigIpaddresses(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        if filename.startswith('records.html') or filename.startswith('preservation'):
            ...
            data_list =[]
            pairs_list = []
            uni = soup.find_all("div", {"id": "property-ip_addresses"})
            ...
            for index, div in enumerate(divs):
                for index, div in enumerate(divs):
                    if index > 1:
                        ...
                        if len(pairs_list) == 3:
                            data_list.append((pairs_list[1],pairs_list[0],pairs_list[2]))
                            pairs_list =[]
            

        if data_list:
            report = ArtifactHtmlReport(f'Facebook & Instagram - Account IP Addresses - {rfilename}')
            report.start_artifact_report(report_folder, f'Facebook Instagram - Account IP Addresses - {rfilename}')
            report.add_script()
            data_headers = ('Timestamp','IP','Action')
            report.write_artifact_data_table(data_headers, data_list, file_found, html_no_escape=['Value'])
            report.end_artifact_report()
            
            tsvname = f'Facebook Instagram - Account IP Addresses - {rfilename}'
            tsv(report_folder, data_headers, data_list, tsvname)
            
            tlactivity = f'Facebook Instagram - Account IP Addresses - {rfilename}'
            timeline(report_folder, tlactivity, data_list, data_headers)
        
        else:
            logfunc(f'No Facebook Instagram - Account IP Addresses - {rfilename}')
