...
def load_profile():
    ...
    destination_path = tk_filedialog.askopenfilename(parent=main_window,
                                                     ...
                                                     filetypes=(('RLEAPP Profile', '*.rlprofile'),))
...
def save_profile():
    ...
    if destination_path:
        ...
        with open(destination_path, 'wt', encoding='utf-8') as profile_out:
            json.dump({'leapp': 'rleapp', 'format_version': 1, 'plugins': selected_modules}, profile_out)
        tk_msgbox.showinfo(
...
def open_lava(project_path, launcher):
    ...
    try:
        ...
        tk_msgbox.showerror(
            ...
            message=f'RLEAPP could not open the project in LAVA:\n{ex}',
            parent=main_window)
        return
    main_window.quit()
...
def open_folder(output_path):
    ...
    try:
        ...
        tk_msgbox.showerror(
            ...
            message=f'RLEAPP could not open the output folder:\n{ex}',
            parent=main_window)
...
def process(casedata):
    ...
    if is_valid:
        ...
        if crunch_successful:
            ...
            completion_button_frame = ttk.Frame(main_window)
            completion_button_frame.grid(ipadx=8)
...
main_window.title(f'RLEAPP version {rleapp_version}')
...
ttk.Separator(bottom_frame, orient='vertical').grid(row=0, column=2, padx=10, sticky='ns')
case_data_button = ttk.Button(bottom_frame, text='Case Data', command=case_data)
case_data_button.grid(row=0, column=3, padx=5)
ttk.Separator(bottom_frame, orient='vertical').grid(row=0, column=4, padx=10, sticky='ns')
...
log_text = tk.Text(
    logtext_frame, name='log_text', bg=theme_inputcolor, fg='black', 
